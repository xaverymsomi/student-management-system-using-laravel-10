                                    ...
                                    <div class="col-lg-9">
                                        <select data-placeholder="Select Teacher" class="form-control select-search" name="teacher_id" id="teacher_id">
                                            ...
                                            @foreach($teachers as $t)
                                                <option {{ old('teacher_id') == App\Helpers\Qs::hash($t->id) ? 'selected' : '' }} value="{{ App\Helpers\Qs::hash($t->id) }}">{{ $t->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                    ...
                    <div class="tab-pane fade" id="c{{ $d->id }}">                         <table class="table datatable-button-html5-columns">
                            ...
                            @foreach($sections->where('my_class.id', $d->id) as $s)
                                <tr>
                                    ...
                                    @if($s->teacher_id)
                                    <td><a target="_blank" href="{{ route('users.show', App\Helpers\Qs::hash($s->teacher_id)) }}">{{ $s->teacher->name }}</a></td>
                                        @else
                                        <td> - </td>
                                    @endif

                            ...
                            <table class="table datatable-button-html5-columns">
                                ...
                                @foreach($used_pins as $up)
                                    <tr>
                                        ...
                                        <td>{{ $up->code }}</td>
                                        <td><a href="{{ $up->user->user_type == 'student' ? route('students.show', App\Helpers\Qs::hash(App\Helpers\Qs::getSRByUserID($up->user->id)->id)) : route('users.show', App\Helpers\Qs::hash($up->user->id)) }}">{{ $up->user->name }}</a></td>
                                        <td>{{ $up->user->user_type }}</td>
                                        <td><a href="{{ route('students.show', App\Helpers\Qs::hash(App\Helpers\Qs::getSRByUserID($up->student->id)->id))  }}">{{ $up->student->name }}</a></td>
                                        <td>{{ $up->updated_at }}</td>
                                    </tr>
                                @endforeach
                                </tbody>

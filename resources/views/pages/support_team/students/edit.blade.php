...
@section('content')
        ...
        <div class="card">
            ...
            <form method="post" enctype="multipart/form-data" class="wizard-form steps-validation ajax-update" data-reload="#ajax-title" action="{{ route('students.update', App\Helpers\Qs::hash($sr->id)) }}" data-fouc>
                @csrf @method('PUT')
                <h6>Personal data</h6>
                <fieldset>
                    <div class="row">
                            ...
                            <div class="form-group">
                                ...
                                <select data-placeholder="Choose..."  name="my_parent_id" id="my_parent_id" class="select-search form-control">
                                    ...
                                    @foreach($parents as $p)
                                        <option {{ (App\Helpers\Qs::hash($sr->parent_id) == App\Helpers\Qs::hash($p->id)) ? 'selected' : '' }} value="{{ App\Helpers\Qs::hash($p->id) }}">{{ $p->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
